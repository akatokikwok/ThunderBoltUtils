...
namespace traffic_manager {
  ...
  void SimpleWaypoint::SetLeftWaypoint(SimpleWaypointPtr &_waypoint) {
    ...
    const cg::Vector3D heading_vector = waypoint->GetTransform().GetForwardVector();//拿到cc::waypoint的前向分量;
    const cg::Vector3D relative_vector = GetLocation() - _waypoint->GetLocation();//拿到入参路点位置和当前路点位置的差向量
  ...
  {
    GeoGridId grid_id;
    if (waypoint->IsJunction()) //如果当前路点恰好是十字路口,就更新junctionID为当前路口的id;否则junctionID更新为默认的类成员geodesic_grid_id
    {
