...
/**
 * 一个RPC客户端，用于与来自Blueprints和UE兼容的C++代码的GRPC服务交互。
* RPC Client用作生成的RPC Client的基类，它包含一组用于管理RPC Channel的方法。
 *
...
class CARFIELDIO_API URpcClient : public UObject
{
	GENERATED_BODY()
...
public:
    ...
    /**
	 * 一个更新函数，用于告诉客户端何时检查查询和分发消息。在widget中（例如），你应该调用它每次更新（或reraw/invalidate）。    
